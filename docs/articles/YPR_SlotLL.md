...
mortality for fish below slot, within slot, and above slot must be
...
fish below the slot and large fish over the slot limit are protected
while harvesting is allowed within the slot limit (i.e., conditional
fishing mortality below `cfBelow` = 0, conditional fishing moratlity in
...
conditional fishing mortality below the slot limit but above recruitment
length `cfBelow`; conditional fishing mortality within the slot limit
...
harvested at if harvest is allowed below the slot limit. The user must
also specify conditional fishing mortality below, within, and above the
slot. Specifying cf is where the user will determine if the slot limit
is a protective slot (set `cfIn` = 0) with harvest below and above the
slot, or an inverse slot (set `cfBelow` and `cfAbove` = 0) with harvest
...
- ndieBelow is the number of fish that die of natural death below the
  slot limit
...
- FBelow is the estimated instantaneous rate of fishing mortality below
  the slot limit
...
- FAbove is the estimated instantaneous rate of fishing mortality above
  the slot limit
- MBelow is the estimated instantaneous rate of natural mortality below
  the slot limit
...
derived from cfBelow, cfIn, and cfOver; recruitmentTL; lowerSL; upperSL;
...
``` r
head(Res_1)
#>   yieldTotal yieldBelow yieldIn yieldAbove nharvTotal ndieTotal nharvestBelow
...
#>   nharvestIn nharvestAbove    n0die ndieBelow   ndieIn ndieAbove  nrBelow
...
#>        nrIn   nrAbove  trBelow     trIn  trOver avglenBelow avglenIn
...
#>   avglenAbove avgwtBelow avgwtIn avgwtAbove   nAt200    nAt250    nAt300
...
#>      nAt325    nAt350  cm  expBelow expIn  expAbove   FBelow FIn    FAbove
...
#>      MBelow       MIn    MAbove    ZBelow       ZIn    ZAbove SBelow SIn SAbove
...
#>   cfBelow cfIn cfOver recruitmentTL lowerSL upperSL  N0 Linf   K   t0 LWalpha
...
multiple slot limits. The last example simulates yield below two
