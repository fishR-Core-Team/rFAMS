...
test_that("ypr_func() output",{
  ...
  expect_equal(names(res1),c("yield","exploitation","Nharvest","Ndie","Nt",
                            "avgwt","avglen","tr","Fmort","Mmort","Zmort","S",
                            "cf","cm","minLL","N0","Linf","K","t0",
                            "LWalpha","LWbeta","maxage","notes"))
...
test_that("ypr_func() results",{
  ...
  expect_equal(round(res1$Fmort,6),ores$Fmort)
  expect_equal(round(res1$Mmort,7),ores$Mmort)
  expect_equal(round(res1$Zmort,7),ores$Zmort)
  expect_equal(round(res1$S,4),ores$S)
  expect_equal(dplyr::select(res1,cf:maxage),dplyr::select(ores,cf:maxage))
})
