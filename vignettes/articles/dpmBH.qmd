...
The `dpmBH_minLL` function will use all the arguments above to simulate the population and generate a list with two data.frame object. The first list item contains a data.frame with a summary by age and the second list item contains a data.frame with a summary by year. The example used here specifies "Striped Bass" that are in the group "landlocked". The species and group are used to assign appropriate PSD categories see [FSA package](https://fishr-core-team.github.io/FSA/index.html) for more information about PSD categories.
