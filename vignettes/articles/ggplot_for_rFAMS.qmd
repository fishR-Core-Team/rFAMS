...
All of the variables returned by `yprBH_MinLL()` are numeric, except for `notes`.
...
The dynamic pool model implemented in `dpmBH_MinLL()` returns an object that is a list of two data.frames, which is more complex than what is returned from other functions in `rFAMS`. However, accessing these two data.frames is fairly simple and is illustrated here.

The code below is copied from this [this article](https://fishr-core-team.github.io/rFAMS/articles/dpmBH.html) that demonstrates how to use `dpmBH_MinLL()`.^[See [the documentation for `dpmBH_MinLL()`](https://fishr-core-team.github.io/rFAMS/reference/dpmBH_MinLL.html) for more details).] The results were assigned to `minLL_2` in the last line.
